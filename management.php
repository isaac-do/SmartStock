...
<body>
    ...
    <div class="dashboard">
        ...
        <table>
            ...
            <tbody>
                <?php
                $sales_rep_search = isset($_GET['sales_rep_search']) ? $_GET['sales_rep_search'] : '';
                if (!empty($sales_rep_search)) {
                    $stmt = $conn->prepare("SELECT * FROM salesrepresentative WHERE SalesRepID LIKE ?");
                    $likeSearch = "%$sales_rep_search%";
                    $stmt->bind_param("s", $likeSearch);
                    $stmt->execute();
                    $sales_result = $stmt->get_result();
                } 
                else
                    $sales_result = $conn->query("SELECT * FROM salesrepresentative");
